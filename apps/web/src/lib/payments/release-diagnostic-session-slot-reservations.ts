...
import {
  ...
  listOpenPaymentTransactionsByDiagnosticSessionIdHex,
...
async function expireOpenPaymentTransactionsForDiagnosticSession(sessionHex: string): Promise<void> {
  const transactions = await listOpenPaymentTransactionsByDiagnosticSessionIdHex(sessionHex);
...
/**
 * Cancels active bookings and open checkout holds linked to a diagnostic session so the time slot is bookable again.
 */
export async function releaseSlotReservationsForDiagnosticSession(diagnosticSessionId: ObjectId): Promise<void> {
  if (!process.env.MONGODB_URI) {
    return;
  }
  const sessionHex = diagnosticSessionId.toString();
  ...
  await expireOpenPaymentTransactionsForDiagnosticSession(sessionHex);
}
