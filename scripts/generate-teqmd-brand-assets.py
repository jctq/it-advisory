...
- **Light** (`teqmd-logo-master.*`): full, compact, mark, SVG wrappers, native app icons.
- **Dark** (`teqmd-logo-master-dark.*`): same crops from the dark-mode master (keys navy backdrop
...
- `teqmd-logo-*.svg` embed the PNGs (not true Bézier SVG—export vectors from design for that).
- **Favicons:** square PNGs at 16–512px from each mark (`teqmd-mark-{size}.png`, `*-dark`).

Requires: pip install pillow numpy

Usage (from repo root):
  python3 scripts/generate-teqmd-brand-assets.py [path-to-light-master]
  python3 scripts/generate-teqmd-brand-assets.py --light [path-to-light-master]
  python3 scripts/generate-teqmd-brand-assets.py --dark [path-to-dark-master]

**Quality ceiling:** raster output cannot exceed the master. For print / retina, add
`teqmd-logo-master.png` at **2400px+** width (lossless) in `_source/` and re-run.
"""
...
def resolve_source_path(cli_path: Path | None) -> Path:
    if cli_path is not None and cli_path.is_file():
        return cli_path
    for name in (
        "teqmd-logo-master.png",
        "teqmd-logo-master.webp",
        "teqmd-logo-master.jpg",
        "teqmd-logo-master.jpeg",
    ):
        candidate = SOURCE_DIR / name
        if candidate.is_file():
            return candidate
    return SOURCE_DIR / "teqmd-logo-master.jpg"
...
def resolve_dark_source_path(cli_path: Path | None) -> Path | None:
    if cli_path is not None and cli_path.is_file():
        return cli_path
    for name in (
        "teqmd-logo-master-dark.png",
        "teqmd-logo-master-dark.webp",
        "teqmd-logo-master-dark.jpg",
        "teqmd-logo-master-dark.jpeg",
    ):
        candidate = SOURCE_DIR / name
        if candidate.is_file():
            return candidate
...
def load_transparent_from_light_backdrop(path: Path) -> Image.Image:
    ...
    edges are not enlarged. Prefer `teqmd-logo-master.png` (lossless) in `_source/`.
    """
...
def build_variants_from_master(
    ...
    *,
    ...
    if label == "light" and is_lossy_raster_source(source):
        print(
            f"Note ({label}): master is JPEG ({source.name})—use teqmd-logo-master.png in "
...
def write_brand_asset_version(web_brand: Path) -> str:
    ...
    parts: list[str] = []
    for path in sorted(SOURCE_DIR.glob("teqmd-logo-master*")):
        if path.is_file():
            stat = path.stat()
...
def main() -> None:
    ...
    if not dark_only:
        ...
        save_png_optimized(full_web, web_brand / "teqmd-logo-full.png")
        save_png_optimized(compact_web, web_brand / "teqmd-logo-compact.png")
        save_png_optimized(mark_web, web_brand / "teqmd-mark.png")
        write_logo_svg(full_web, "teqmd-logo-full.png", web_brand / "teqmd-logo-full.svg")
        write_logo_svg(compact_web, "teqmd-logo-compact.png", web_brand / "teqmd-logo-compact.svg")
        write_mark_favicons(mark_web, "teqmd-mark", web_brand)
    ...
    if dark_source is not None and dark_source.is_file():
        ...
        save_png_optimized(full_dark_web, web_brand / "teqmd-logo-full-dark.png")
        save_png_optimized(compact_dark_web, web_brand / "teqmd-logo-compact-dark.png")
        save_png_optimized(mark_dark_web, web_brand / "teqmd-mark-dark.png")
        write_logo_svg(
            full_dark_web,
            "teqmd-logo-full-dark.png",
            web_brand / "teqmd-logo-full-dark.svg",
        )
        write_logo_svg(
            compact_dark_web,
            "teqmd-logo-compact-dark.png",
            web_brand / "teqmd-logo-compact-dark.svg",
        )
        write_mark_favicons(mark_dark_web, "teqmd-mark-dark", web_brand)
        ...
        save_png_optimized(
            ...
            web_brand / "teqmd-logo-full-dark.png",
        )
        save_png_optimized(
            supersampled_recolor_for_dark_ui(compact_web, DARK_RECOLOR_SUPERSAMPLE_FACTOR),
            web_brand / "teqmd-logo-compact-dark.png",
        )
        mark_dark_fallback = supersampled_recolor_for_dark_ui(mark_web, DARK_RECOLOR_SUPERSAMPLE_FACTOR)
        save_png_optimized(mark_dark_fallback, web_brand / "teqmd-mark-dark.png")
        write_mark_favicons(mark_dark_fallback, "teqmd-mark-dark", web_brand)
        print("Dark assets from programmatic recolor (no dark master in _source/)", file=sys.stderr)
    elif dark_only:
        print("Dark source not found in _source/ (teqmd-logo-master-dark.*)", file=sys.stderr)
        sys.exit(1)
    elif light_only and full_web is None:
        print("Light source not found in _source/ (teqmd-logo-master.*)", file=sys.stderr)
        sys.exit(1)
