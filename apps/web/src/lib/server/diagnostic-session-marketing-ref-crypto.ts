...
import {
  LEGACY_MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX,
  MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX,
...
function resolveEncryptionKey(): Buffer | null {
  const raw =
    process.env.DIAGNOSTIC_SESSION_URL_SECRET?.trim() ??
    process.env.DIAGNOSTIC_SESSION_URL_SECRET?.trim() ??
    '';
...
/**
 * When {@link DIAGNOSTIC_SESSION_URL_SECRET} is configured, returns an opaque URL-safe token for this diagnostic session row.
 * Otherwise returns the hex id unchanged (legacy / local dev).
 */
export function encodeDiagnosticSessionRefForMarketingUrl(objectIdHex24: string): string {
  if (!/^[a-f\d]{24}$/i.test(objectIdHex24)) {
    throw new Error('Invalid diagnostic session object id');
  }
  ...
  return `${MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX}${combined.toString('base64url')}`;
}

/**
 * Resolves a marketing ref or raw ObjectId hex to a lowercase 24-char hex string. Returns null when invalid.
 */
export function resolveDiagnosticSessionObjectIdHexFromMarketingRef(ref: string): string | null {
  const trimmed = ref.trim();
  if (/^[a-f\d]{24}$/i.test(trimmed)) {
    return trimmed.toLowerCase();
  }
  const prefix = trimmed.startsWith(MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX)
    ? MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX
    : trimmed.startsWith(LEGACY_MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX)
      ? LEGACY_MARKETING_DIAGNOSTIC_SESSION_REF_PREFIX
      : null;
  if (prefix === null) {
    return null;
