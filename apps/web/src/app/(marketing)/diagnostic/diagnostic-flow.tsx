...
import {
  ...
  buildMarketingDiagnosticRetakePath,
...
export function DiagnosticFlow(props: DiagnosticFlowProps = {}): ReactElement {
  ...
  useLayoutEffect(() => {
    ...
    router.replace(`${buildMarketingDiagnosticSessionPath(fromQuery)}${retakeSuffix}`);
  ...
  const sessionTargetId = useMemo((): string | null => {
    ...
    return isPlausibleMarketingDiagnosticSessionRef(raw) ? raw : null;
  ...
  useEffect(() => {
    ...
    const nextPath = buildMarketingDiagnosticSessionPath(persistedSessionRef);
  ...
  useEffect(() => {
    ...
    async function initializeSession(): Promise<void> {
      ...
      if (isRetakeQuery) {
        ...
        const nextPath =
          sessionTargetId !== null ? buildMarketingDiagnosticSessionPath(sessionTargetId) : '/diagnostic';
      ...
      try {
        const sessionUrl =
          sessionTargetId !== null
            ? `${DIAGNOSTIC_SESSION_API_URL}?sessionId=${encodeURIComponent(sessionTargetId)}`
            : DIAGNOSTIC_SESSION_API_URL;
        ...
        if (response.ok && sessionTargetId === null) {
          ...
          if (peekReadOnly) {
            const forkResponse = await fetch(DIAGNOSTIC_SESSION_API_URL, {
              method: 'DELETE',
              credentials: 'include',
            });
            if (forkResponse.ok) {
        ...
        capturePersistedSessionRef(parseDiagnosticSessionIdFromApiPayload(data));
  ...
  const executeJumpToRoundProgressStep = useCallback(
    (targetRoundStepIndex: number) => {
      ...
      } else if (targetRoundStepIndex < 0 || targetRoundStepIndex >= visibleTemplateRounds.length) {
        return;
      }
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            {showRetakeLink && !sessionReadOnly ? (
              <Button type="button" variant="outline" asChild>
                <Link href={buildMarketingDiagnosticRetakePath(marketingSessionRef)}>Retake diagnostic</Link>
              </Button>
            ) : null}
            {canDeleteDiagnostic ? (
              <>
