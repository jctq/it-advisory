...
import {
  ...
  listDiagnosticSessionsForVisitorPaginated,
...
/**
 * Lists diagnostic session snapshots for the signed-in marketing account with server-side pagination and filters.
 */
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const result = await listDiagnosticSessionsForVisitorPaginated({
    visitorId,
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const visitorId = buildAccountVisitorId(user.id);
  const sessionId = await insertBlankDiagnosticSessionForVisitor(visitorId);
  if (sessionId === null) {
    return NextResponse.json({ error: 'Database unavailable' }, { status: 503 });
  }
  return NextResponse.json({ sessionId: encodeDiagnosticSessionRefForMarketingUrl(sessionId) });
}
