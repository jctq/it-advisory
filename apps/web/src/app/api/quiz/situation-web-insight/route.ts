...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await generateText({
      ...
      temperature: 0.35,
      system:
        'You support visitors to a Philippines-focused independent IT advisory practice. Use web search when it improves factual grounding (products, vendors, trends). Stay neutral—no legal, medical, or HR guarantees. Short paragraphs only.',
      prompt: `The visitor typed this situation or topic:\n"${trimmed}"\n\nWrite at most 130 words. Explain briefly what organizations often run into in similar situations (generally, not naming the visitor). Where web sources helped, answer in plain language. End with one sentence on what is useful to clarify in an advisory session. If search adds little value, say so briefly and reason from general IT advisory context.`,
    });
